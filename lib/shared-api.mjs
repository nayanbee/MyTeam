...
export async function updateInvitedPassword(accessToken, password) {
  const response = await fetch(`${apiBase()}/auth/v1/user`, {
    method:'PUT', headers:{apikey:anonKey(),Authorization:`Bearer ${accessToken}`,'Content-Type':'application/json'},
    body:JSON.stringify({password})
  });
  const result = await response.json().catch(() => ({}));
  return {ok:response.ok,error:result.msg || result.error_description || result.error || 'Could not set password'};
}

...
4. In the Vercel project, set `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` as server environment variables for the desired environment; redeploy. The API also accepts a legacy `SUPABASE_ANON_KEY`, but new projects should use the publishable key. These values are read only by `/api/*`. Do not put a secret key or database password in a `VITE_` variable.
